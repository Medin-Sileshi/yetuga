...
class CacheManager {
  ...
  static const int priorityHigh = 3;   // Critical data, keep longest
  static const int priorityMedium = 2; // Important data
  static const int priorityLow = 1;    // Nice to have data

  // Cache expiration times
  static const Duration highExpiration = Duration(days: 7);
  static const Duration mediumExpiration = Duration(days: 2);
  static const Duration lowExpiration = Duration(hours: 12);
  ...
  Future<void> put(String key, dynamic data, {int priority = priorityMedium, bool persistToDisk = true}) async {
    try {
  ...
  Duration _getExpirationTime(int priority) {
    switch (priority) {
      case priorityHigh:
        return highExpiration;
      case priorityMedium:
        return mediumExpiration;
      case priorityLow:
        return lowExpiration;
      default:
        return mediumExpiration;
    }
  }
  ...
  Future<_CacheEntry?> _getFromDisk<T>(String key) async {
    try {
      ...
      // We don't need lastAccessed here as we're updating it anyway
  ...
  void _enforceMemoryCacheLimit() {
    ...
    final entriesToRemove = entries.length - maxMemoryCacheSize;
  ...
  Future<void> _enforceDiskCacheLimit() async {
    try {
      ...
      if (metadataBox.length <= maxDiskCacheSize) {
        return;
      }
      ...
      // Remove oldest entries until we're under the limit
      final entriesToRemove = entries.length - maxDiskCacheSize;
      final prefs = await SharedPreferences.getInstance();
