...
class _CreateAccountScreenState extends ConsumerState<CreateAccountScreen> {
  ...
  Future<void> _createAccount() async {
    ...
    try {
      // First, clear any existing onboarding data to prevent using data from previous users
      final storageService = ref.read(storageServiceProvider);
      try {
        await storageService.clearAllOnboardingData();
        Logger.d('CreateAccountScreen', 'Cleared ALL existing onboarding data');
      } catch (e) {
        Logger.d('CreateAccountScreen', 'Error clearing onboarding data: $e');
      ...
      if (mounted && userCredential.user != null) {
        // Force a refresh of the onboarding provider with the new user
        try {
          await ref.read(onboardingProvider.notifier).clearData();
          Logger.d('CreateAccountScreen', 'Cleared onboarding provider data');
        } catch (e) {
          Logger.d('CreateAccountScreen', 'Error clearing onboarding provider: $e');
          // Continue even if clearing fails
        }

        // Log the current user ID for debugging
        final userId = userCredential.user!.uid;
        Logger.d('CreateAccountScreen', 'New account created with user ID: $userId');
        ...
        if (mounted) {
          ...
          // Always navigate to the onboarding screen for new accounts
